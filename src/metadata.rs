...
pub trait MetadataStore: Send + Sync {
    ...
    async fn multi_get(
        &self,
        keys: &[CompositeKey],
    ) -> Result<HashMap<CompositeKey, Vec<u8>>, MetadataError>;
...
impl MetadataStore for SqliteMetadataStore {
    async fn get(&self, key: &CompositeKey) -> Result<Option<Vec<u8>>, MetadataError> {
        ...
        let mut stmt =
            conn.prepare("SELECT value FROM metadata WHERE partition_key = ?1 AND sort_key = ?2")?;
    ...
    async fn multi_get(
        ...
        for key in keys {
            let mut stmt = conn
                .prepare("SELECT value FROM metadata WHERE partition_key = ?1 AND sort_key = ?2")?;
            let mut rows = stmt.query(params![key.partition_key, key.sort_key])?;
        ...
        for op in operations {
            match op {
                BatchOp::Put(key, value) => {
                    tx.execute(
                        "INSERT OR REPLACE INTO metadata (partition_key, sort_key, value) VALUES \
                         (?1, ?2, ?3)",
                        params![key.partition_key, key.sort_key, value],
                    )?;
                },
