...
#[tokio::test]
async fn test_forest_based_compactor_with_mock() {
    requires_docker!();
    let metadata_store = setup_test_db().await.expect("Failed to setup test DB");
    ...
    // Create a test table
    let table_name = TableName::from("test_forest_compactor");
    let table_config = TableConfig {
        table_id: None,
        table_name: table_name.clone(),
    };
    ...
    let test_runs = vec![
        (
            RunID::from("test_run_1"),
            Stats::StatsV1(StatsV1 {
                min_key: "key1".to_string(),
                max_key: "key100".to_string(),
                size_bytes: 1000,
                put_count: 50,
                delete_count: 5,
            }),
        ...
        (
            RunID::from("test_run_2"),
            Stats::StatsV1(StatsV1 {
                min_key: "key2".to_string(),
                max_key: "key200".to_string(),
                size_bytes: 2000,
                put_count: 100,
                delete_count: 10,
            }),
    ...
    metadata_store
        .append_wal(test_runs.clone())
        .await
        .expect("Failed to append WAL runs");

    // Create forest and verify we can get WAL runs (same logic as compactors)
    let snapshot = ForestImpl::latest(metadata_store, object_store)
        .await
        .expect("Failed to create forest snapshot");

    assert_eq!(snapshot.wal.len(), 2, "Should have 2 WAL runs in forest");

    // Test the same logic as compactors: take up to 16 runs
    let count = std::cmp::min(16, snapshot.wal.len());
    assert_eq!(count, 2);

    let runs_to_compact: Vec<RunID> = snapshot
        .wal
        .values()
        .take(count)
        .map(|metadata| metadata.id.clone())
    ...
    assert_eq!(runs_to_compact.len(), 2);
    assert!(runs_to_compact.contains(&RunID::from("test_run_1")));
    assert!(runs_to_compact.contains(&RunID::from("test_run_2")));

    println!("Forest-based compactor with mock test completed successfully!");
}
